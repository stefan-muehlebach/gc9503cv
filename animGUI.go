...
func NewGUI(bounds geom.Rectangle[int]) *GUI {
	...
	main.Add(btnA, btnB, btnC, NewSpacer())
	...
	toolBar.Add(icn1, icn2, icn3, icn4, icn5, icn6, icn7)

	main.Add(toolBar, NewSpacer())

	chk1 := NewCheckbox("Knoblibrot")
	chk2 := NewCheckbox("Güggeli im Chörbli")
	chk3 := NewCheckbox("Hummus (scharf)")
	chk4 := NewCheckbox("Döner im Fladebrot")

	main.Add(chk1, chk2, chk3, chk4, NewSpacer())

	rad1 := NewRadiobutton("Zum selber abholen")
	rad2 := NewRadiobutton("Lieferung durch eigenen Lieferservice")
	rad3 := NewRadiobutton("Per Post Prioritaire")
	rad4 := NewRadiobutton("Postlagernd")

	main.Add(rad1, rad2, rad3, rad4)
