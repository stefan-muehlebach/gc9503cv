...
func BindStruct(i interface{}) Struct {
	...
	if t.Kind() != reflect.Ptr ||
		...
		log.Println("Invalid type passed to BindStruct, must be pointer to struct")
		return NewUntypedMap().(Struct)
	}
...
func (b *boundStruct) Reload() (retErr error) {
	...
	for j := 0; j < v.NumField(); j++ {
		...
		if kind == reflect.Slice || kind == reflect.Struct {
			log.Println("Data binding does not yet support slice or struct elements in a struct")
			continue
		}
