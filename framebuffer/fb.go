...
func Open(device string) (*Device, error) {
	...
	if varInfo.red.offset == 11 && varInfo.red.length == 5 &&
		varInfo.red.msb_right == 0 && varInfo.green.offset == 5 &&
		varInfo.green.length == 6 && varInfo.green.msb_right == 0 &&
		varInfo.blue.offset == 0 && varInfo.blue.length == 5 &&
...
func (d *Device) At(x, y int) color.Color {
	if x < d.Rect.Min.X || x >= d.Rect.Max.X ||
		y < d.Rect.Min.Y || y >= d.Rect.Max.Y {
		return rgb565(0)
	}
	i := y*d.Stride + 2*x
	return rgb565(d.Pix[i+1])<<8 | rgb565(d.Pix[i])
}

// Set implements the draw.Image interface.
func (d *Device) Set(x, y int, c color.Color) {
	// the min bounds are at 0,0 (see Open)
	if x >= 0 && x < d.Rect.Max.X &&
		y >= 0 && y < d.Rect.Max.Y {
		r, g, b, a := c.RGBA()
		if a > 0 {
			rgb := toRGB565(r, g, b)
			...
			// Raspbian. The d.Pix indices have to be swapped if the target
			// system is big endian.
			d.Pix[i+1] = byte(rgb >> 8)
			d.Pix[i] = byte(rgb & 0xFF)
		}
	}
}

// Verlangt aktuell noch, dass sowohl der Framebuffer als auch img
// die gleiche Groesse haben.
/*
func (d *Device) Send(img image.Image) error {
	rgbImg, isRGBA := img.(*image.RGBA)
	if !isRGBA {
		return errors.New("Only RGBA images can be sent directly")
	}
	if d.Bounds() != rgbImg.Bounds() {
		return errors.New("Image must have the same bounds as the screen")
	}
	srcBytesPerPixel := 4
	dstBytesPerPixel := 2
	srcBaseIdx := 0
	dstBaseIdx := 0
	for range rgbImg.Bounds().Dy() {
		srcIdx := srcBaseIdx
		dstIdx := dstBaseIdx
		for range rgbImg.Bounds().Dx() {
			src := rgbImg.Pix[srcIdx : srcIdx+srcBytesPerPixel : srcIdx+srcBytesPerPixel]
			dst := d.Pix[dstIdx : dstIdx+dstBytesPerPixel : dstIdx+dstBytesPerPixel]
			r := src[0] & 0xF8
			g := src[1] & 0xFC
			b := src[2] & 0xF8
			dst[1] = (r) | (g >> 5)
